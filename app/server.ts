...
Server.prototype.start = async function start() {
	...
	try{
		...
		this.bonjour.publish({ name: `${os.hostname()}-${(Math.random() + 1).toString(36).substring(7)}`, type: 'mtgoverlay', port: this.port, txt: this.about });
	} catch (err){
	console.error(err)
	}
	...
	this.server.get('/:filename', (req, res, next) => {
		try {
			...
			res.write('<title>MTGOverlay | ' + (this.themeManifest.name || this.theme) + ' | ' + req.params.filename + ' ' + (this.themeManifest.resolution ? '[' + self.themeManifest.resolution.join(",") + ']' : '') + '</title>\r\n');
	...
	this.server.get('/', (req, res) => {
		...
		fs.readdir(path.join(this.webPath, 'themes/' + this.theme), (err, files) => {
			...
			res.write('<html><head><title>' + this.theme + ' | stream overlays</title><meta charset="UTF-8" />');
			res.write('<link rel="icon" type="image/x-icon" href="/favicon.svg">');
			res.write(`<style>
			body {
					...
					case 'subscribe':
						switch(inData.data.type){
							case 'dmx':
								...
								subscriptionCalls = ['dmx-channel', 'scoreboard'];
								break;
							case 'app':
